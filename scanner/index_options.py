...
def recommend_index_options(index_symbol: str, df: pd.DataFrame, features: dict) -> list[IndexOptionPlan]:
    ...
    index_symbol should be the clean uppercase tag (e.g. "NIFTY",
    "BANKNIFTY") - the mapping to the broker's actual instrument name
    (e.g. "Nifty 50") happens earlier, when the data was FETCHED (see
    main.py's _get_angelone_mapped_symbol), not here.
    """
    plans: list[IndexOptionPlan] = []

    if df is None or df.empty:
        logger.warning(f"Empty data received for {index_symbol}, skipping options setup")
        return plans
